...
const MyPetInfo = ({ route, navigation }) => {
  ...
  const fetchBreedInfo = async () => {
    setLoading(true);
    try {
      const response = await axios.post('http://192.168.3.51/dogcare/getbreedinfo.php', { breed_id: pet.breed_id });
  ...
  const handleSave = async () => {
    try {
      const response = await axios.post('http://192.168.3.51/dogcare/editpetinfo.php', {
        pet_id: pet.pet_id,
        ...updatedPet,
      });
      ...
      if (response.data.success) {
        ...
        setIsEditing(false);
        const updatedPetResponse = await axios.post('http://192.168.3.51/dogcare/getpets.php', { pet_id: pet.pet_id });
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              const response = await axios.post('http://192.168.3.51/dogcare/deletepet.php', { pet_id: pet.pet_id });
              if (response.data.success) {
