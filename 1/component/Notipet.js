...
const NotiPet = ({ route }) => {
  ...
  useEffect(() => {
    const fetchPetNotification = async () => {
      try {
        const response = await axios.post('http://192.168.3.51/dogcare/notipet.php', { pet_id });
  ...
  const handleAddNotification = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.3.51/dogcare/addnoti.php', {
        noti_name: notiName,
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              const response = await axios.post('http://192.168.3.51/dogcare/deletenoti.php', {
                noti_id,
              });
